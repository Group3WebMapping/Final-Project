...
// Create the map with a streets-navigation-vector basemap
 ...
 // Create the view for the map, setting initial center and zoom
  ...
  // Add a 2010 demographic feature layer
 ...
 // Listen for form submission to perform a query
  queryForm.addEventListener("submit", function (event) {
    ...
    if (populationInput) {
      whereClause += ` AND POP2010 >= ${parseFloat(populationInput)}`;
    }
    if (incomeInput) {
      whereClause += ` AND Median_Income >= ${parseFloat(incomeInput)}`;
    }
    if (medianAgeInput) {
      whereClause += ` AND Median_Age_2010 <= ${parseFloat(medianAgeInput)}`;
    }
    if (occupiedHousingInput) {
      whereClause += ` AND F__Occupied_Housing >= ${parseFloat(occupiedHousingInput)}`;
    }
